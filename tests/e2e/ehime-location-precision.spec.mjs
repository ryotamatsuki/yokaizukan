...
test('desktop: 11伝承markerはPhase A common projectionとPhase B precision metadataを使う', async ({ page }) => {
  ...
  const state = await markers.evaluateAll((nodes) => nodes.map((node) => ({
    id: node.dataset.legendId, precision: node.dataset.locationPrecision, role: node.dataset.coordinateRole,
    source: node.dataset.projectionSource, x: Number(node.dataset.projectedX), y: Number(node.dataset.projectedY),
    left: parseFloat(node.style.left), top: parseFloat(node.style.top)
  })));
  expect(new Set(state.map((item) => item.id)).size).toBe(11);
  for (const item of state) {
    expect(item.precision).toBeTruthy(); expect(item.role).toBeTruthy(); expect(item.source).toBe('local-n03-2026');
    expect(Number.isFinite(item.x)).toBe(true); expect(Number.isFinite(item.y)).toBe(true);
    expect(item.left).toBeGreaterThanOrEqual(0); expect(item.left).toBeLessThanOrEqual(100);
    expect(item.top).toBeGreaterThanOrEqual(0); expect(item.top).toBeLessThanOrEqual(100);
  }
  ...
  expect(runtime.externalGeoRequests).toEqual([]); expect(runtime.pageErrors).toEqual([]); expect(runtime.consoleErrors).toEqual([]);
...
test('geoDebug: Phase A 20 anchorsとPhase B 11 legend anchorsを同一SVGで表示する', async ({ page }) => {
  ...
  await expect(debug).toHaveAttribute('data-phase', 'A'); await expect(debug).toHaveAttribute('data-location-phase', 'B');
  await expect(debug).toHaveAttribute('data-municipality-count', '20'); await expect(debug).toHaveAttribute('data-legend-count', '11');
  await expect(debug.locator('.ehime-hall-anchor')).toHaveCount(20); await expect(debug.locator('.ehime-legend-geo-anchor')).toHaveCount(11);
  await expect(page.locator('#mapLegendList .map-debug-list__item')).toHaveCount(20); await expect(page.locator('#mapLegendList .map-debug-list__legend')).toHaveCount(11);
...
test('390px smartphone: Phase B markerとprecision UIはoverflowせずtouch操作を維持する', async ({ page }) => {
  ...
  await enterMap(page);
  const map = page.locator('#ehimeMap');
  await expect(map).toHaveAttribute('data-legend-geography-phase', 'B'); await expect(map.locator('.map-marker')).toHaveCount(11);
  ...
  const boxes = await map.locator('.map-marker').evaluateAll((nodes) => nodes.map((node) => { const r = node.getBoundingClientRect(); return { left: r.left, right: r.right }; }));
  for (const box of boxes) { expect(box.right).toBeGreaterThan(0); expect(box.left).toBeLessThan(390); }
  const marine = map.locator('[data-legend-id="uwakai_sea_mystery_cluster"]');
  await expect(marine).toBeVisible(); await marine.click();
  await expect(page.locator('#legendModal')).toHaveAttribute('aria-hidden', 'false');
  await expect(page.locator('#legendModal .legend-modal__panel')).toBeVisible();
